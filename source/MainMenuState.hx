...
class MainMenuState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		logo.updateHitbox();
		logo.antialiasing = FlxG.save.data.lowquality;
		add(logo);
		...
		sketch.antialiasing = FlxG.save.data.lowquality;
		add(sketch);
		...
		if (GameJoltAPI.getStatus())
		{
			...
			if (GameJoltAPI.getUserInfo().contains('ronyx'))
			{
				versionText.text += '  //  Thanks for being ronyx!';
			}
			if (GameJoltAPI.getUserInfo() == 'ronyx_04')
			{
				versionText.text += '  //  che palle';
			}
		}
